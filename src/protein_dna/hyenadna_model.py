...
import os,re,json,subprocess,transformers,math,torch
...
def load_fasta(file_path, logger):
...
def save_embeddings(names, embeddings, output_path, logger):
...
def pad_sequences(sequences, pad_char, max_len):
    padded_sequences = [seq + pad_char * (max_len - len(seq)) for seq in sequences]
    return padded_sequences

def main(sequence_file, output_file, device_id, batch_size):
    log_file = 'process.log'
    logger = setup_logger(log_file)
    
    logger.info(f"Starting embedding calculation for sequences in {sequence_file} using device cuda:{device_id}")

    # Select pretrained model and configuration
    ...
    logger.info(f"Loading model {pretrained_model_name} to device cuda:{device_id}")
    if pretrained_model_name in max_lengths:
        model = HyenaDNAPreTrainedModel.from_pretrained(
            str(Path(os.environ.get('PREMPNI_MODEL_ROOT', '/opt/prempni/models')) / 'hyenadna'),
    ...
    names, sequences = load_fasta(sequence_file, logger)

    # Calculate the maximum length of the sequences
    max_seq_length = max(len(seq) for seq in sequences)
    if max_seq_length > max_length:
        raise ValueError(f"Sequence length {max_seq_length} exceeds the maximum length {max_length}")

    # Pad sequences to the maximum length with 'N'
    padded_sequences = pad_sequences(sequences, 'N', max_seq_length)
    ...
    with torch.inference_mode():
        for i in tqdm(range(0, len(sequences), batch_size), desc="Processing batches"):
            ...
            # Tokenize sequences
            tokenized_seqs = [tokenizer(seq)["input_ids"] for seq in batch_seqs]
            tokenized_seqs = torch.LongTensor(tokenized_seqs).to(device)

            # Get embeddings
            embeddings = model(tokenized_seqs)
    ...
    save_embeddings(names, all_embeddings, output_file, logger)
    logger.info(f"Embeddings saved to {output_file}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Calculate embeddings for DNA sequences in a FASTA file')
    parser.add_argument('sequence_file', type=str, help='Path to the input FASTA file containing DNA sequences')
    ...
    parser.add_argument('--batch_size', type=int, default=100, help='Batch size for processing sequences (default: 100)')
    args = parser.parse_args()
    main(args.sequence_file, args.output_file, args.device, args.batch_size)
