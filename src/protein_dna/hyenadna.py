...
def normalize_device(device: str) -> torch.device:
    ...
    if torch_device.type == "cuda":
        ...
        if index >= torch.cuda.device_count():
            raise RuntimeError(
                f"CUDA device {index} does not exist; available GPU count: {torch.cuda.device_count()}."
            )
    return torch_device
...
class HyenaDNAEmbedder:
    ...
    def __init__(
        ...
        if not checkpoint.is_file():
            raise FileNotFoundError(f"HyenaDNA checkpoint not found: {checkpoint}")
    ...
    def embed_chain(self, chain: DNAChain) -> torch.Tensor:
        ...
        if len(token_ids) != expected_token_count:
            raise RuntimeError(
                f"DNA chain {chain.chain_id} has an unexpected token count: expected "
                f"{expected_token_count}, got {len(token_ids)}."
            )
        ...
        if tuple(embedding.shape) != expected_shape:
            raise RuntimeError(
                f"DNA chain {chain.chain_id} has an unexpected output shape: expected {expected_shape}, "
                f"got {tuple(embedding.shape)}."
            )
        if not torch.isfinite(embedding).all():
            raise RuntimeError(f"DNA chain {chain.chain_id} embeddings contain NaN or Inf.")
        return embedding
