...
| `run_dna.sh`, `run_rna.sh` | CPU-default commands for v0.2.0 |

The JSON and TSV were compared with the public website downloads on 2026-09-10. They are also bundled at `/opt/prempni/examples/` in the image. v0.2.0 accepts them directly with `--input-json` or `--input-tsv`. Both shell scripts use the same inputs and the image's CPU defaults.
